...
function SharkAttacksHeader(props) {
  ...
  function handleImport() {
    setImporting(true);
    dispatch(Actions.importSharkAttacks(user.selectedOrganization.id)).finally(
      () => setImporting(false),
    );
  }
  ...
  return (
    <div className="flex flex-1 w-full items-center justify-between">
      ...
      <FuseAnimate animation="transition.slideRightIn" delay={300}>
        ...
        >
          {importing ? (
            <Icon className="mr-4 text-16 animate-spin">autorenew</Icon>
          ) : (
            <Icon className="mr-4 text-16">cloud_download</Icon>
          )}
          <span>{T.translate("shark_attacks.import_button")}</span>
        </Button>
      </FuseAnimate>
      <FuseAnimate animation="transition.slideRightIn" delay={300}>
        <Button
