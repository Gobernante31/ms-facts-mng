...
function SharkAttack(props) {
  ...
  // Related cases (Part 3)
  const relatedCases = useSelector(
    ({ SharkAttackManagement }) =>
      SharkAttackManagement.sharkAttacks.relatedCases,
  );
  const relatedCasesLoading = useSelector(
    ({ SharkAttackManagement }) =>
      SharkAttackManagement.sharkAttacks.relatedCasesLoading,
  );
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      let errMessage;
      if (networkError) {
        errMessage = JSON.stringify(networkError);
      } else if (graphQLErrors.length === 0) {
        errMessage = message;
      } else {
        errMessage = graphQLErrors[0].message.name;
      }
      dispatch(
        AppActions.showMessage({
          message: errMessage,
  ...
  /**
   * Handle the "Consult more cases in [Country]" button.
   */
  function handleConsultMore() {
    dispatch(Actions.getRelatedSharkAttacks(form.country));
  }
          ...
          <div className="p-16 sm:p-24 max-w-2xl">
                ...
                return (
                  <form noValidate onSubmit={handleSubmit}>
                    {tabValue === 0 && (
                      <BasicInfo
                        dataSource={values}
                        {...{ T, onChange, canWrite, errors, touched }}
                      />
                    )}
            ...
            {form.country ? (
              <div className="mt-16">
                ...
                >
                  {T.translate("shark_attack.consult_more_cases", {
                ...
                {relatedCasesLoading && (
                  <div className="mt-16">
                    <Typography variant="caption">
                      {T.translate("shark_attack.loading_related")}
                    </Typography>
                    <LinearProgress />
                  </div>
                )}
                {relatedCases.length > 0 && (
                  <div className="mt-16">
                    <Typography variant="h6">
                      {T.translate("shark_attack.related_cases_title")}
                    </Typography>
                    <List dense>
                      {relatedCases.map((c, i) => {
                        const primary = [c.date, c.type, c.country]
                          .filter(Boolean)
                          .join(" · ");
                        const secondary = [
                          c.name,
                          ...
                          .join(" — ");
                        return (
                          <ListItem key={c.id || c.date} divider>
                            <ListItemText
                              primary={primary}
                              secondary={
                                c.href ? `${secondary} (${c.href})` : secondary
                              }
                            />
                          </ListItem>
                        );
